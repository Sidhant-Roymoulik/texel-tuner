...
static constexpr int32_t mg_score(int32_t score) { return static_cast<int16_t>(score); }

static constexpr int32_t eg_score(int32_t score) { return static_cast<int16_t>((score + 0x8000) >> 16); }
#else
constexpr int32_t S(const int32_t mg, const int32_t eg) { return (mg + eg) / 2; }
#endif

template <typename T>
void get_initial_parameter_single(parameters_t& parameters, const T& parameter) {
#if TAPERED
    const tune_t mg   = mg_score(static_cast<int32_t>(parameter));
    const tune_t eg   = eg_score(static_cast<int32_t>(parameter));
